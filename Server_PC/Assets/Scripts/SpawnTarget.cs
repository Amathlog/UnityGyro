...
public class SpawnTarget : MonoBehaviour {
	...
	public void Spawn(Vector3 position, Color color) {
        ...
        float yValue = (position.y - minY) / (maxY - minY);

        //obj.transform.position = position;
        obj.GetComponent<Rigidbody>().AddForce(Vector3.forward * forwardForce +  (Vector3.right * xValue * ratio + Vector3.up * yValue * invRatio * 2.0f) * planeForce);
